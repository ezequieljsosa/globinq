...
if __name__ == '__main__':
    ...
    df = pd.read_table("data/generated/blast_pdb.tbl", sep="\t",
                       names=["query", "hit", "identity", "4", "5", "6", "7", "8", "9", "10", "11"], index_col=False)
    ...
    for tax in tqdm(tax_cepa):
        ...
        g = Globin()
        g.tax = Tax.select().where(Tax.id == tax[:-1]).get()
        ...
        if tax in tax_unip:
            ...
            print tax + " sin uniprot "
            active_site = active_site_res[map(lambda x: tax_cepa[tax] in x, active_site_res.tax)]
        ...
        pdbs["chain"] = [x.split("_")[1] for x in pdbs.hit]
        pdbs["cod"] = [x.split("_")[0] for x in pdbs.hit]
